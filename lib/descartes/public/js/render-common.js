...
var myGraphOptions = 'template=' + myGraphTemplate;
var myPageIndex = 1;
var myRowIndex = 0;
...
var myYMaxRight = '';
var myDescription = '';
var myGraphType = 'line';
var myLineMode = 'slope';
var myAreaMode = 'off';
var myLogMode = 'off';
var myPieMode = 'average';
var myNullMode = 'true';
var myHideLegend = 'false';
var myHideGrid = 'false';
var myHideAxes = 'false';
var myHideYAxis = 'false';
var myGraphRefreshId = 0;
var myGraphRefreshInterval = 60000;
...
var updateLocalSettings = function(settings) {
  if (settings !== null) {
    if (typeof settings.titleLeft === 'string') { myTitleLeft = settings.titleLeft; }
    if (typeof settings.titleRight === 'string') { myTitleRight = settings.titleRight; }
    if (typeof settings.ymaxLeft === 'string') { myYMaxLeft = settings.ymaxLeft; }
    if (typeof settings.ymaxRight === 'string') { myYMaxRight = settings.ymaxRight; }
    if (typeof settings.graphType === 'string') { myGraphType = settings.graphType; }
    ...
    if (typeof settings.pieMode === 'string') { myPieMode = settings.pieMode; }
    ...
    if (typeof settings.hideLegend === 'string') { myHideLegend = settings.hideLegend; }
    if (typeof settings.hideGrid === 'string') { myHideGrid = settings.hideGrid; }
    if (typeof settings.hideAxes === 'string') { myHideAxes = settings.hideAxes; }
    if (typeof settings.hideYAxis === 'string') { myHideYAxis = settings.hideYAxis; }
  }
};
...
var constructGraphUrl = function(graph) {
  ...
  var myGraphUrl = '/render/?' + 'height=' + myGraphHeight + '&width=' + myGraphWidth;
  var targets = '';
  ...
  myGraphUrl += '&from=-' + myInterval + targets;
  myLocalGraphType = (graph.graphType || myGraphType);
  if (myLocalGraphType !== 'line') {
    myGraphUrl += '&graphType=' + myLocalGraphType;
  }
  myLocalLineMode = (graph.lineMode || myLineMode);
  if (myLocalLineMode !== 'slope') {
    myGraphUrl += '&lineMode=' + myLocalLineMode;
  }
  myLocalAreaMode = (graph.areaMode || myAreaMode);
  if (myLocalAreaMode !== 'off') {
    myGraphUrl += '&areaMode=' + myLocalAreaMode;
  }
  myLocalPieMode = (graph.pieMode || myPieMode);
  if (myLocalPieMode !== 'average') {
    myGraphUrl += '&pieMode=' + myLocalPieMode;
  }
  ...
  if (myLocalNullMode !== 'false') {
    myGraphUrl += '&drawNullAsZero=true';
  }
  myLocalHideLegend = (graph.hideLegend || myHideLegend);
  myGraphUrl += '&hideLegend=' + myLocalHideLegend;
  myLocalHideGrid = (graph.hideGrid || myHideGrid);
  if (myLocalHideGrid !== 'false') {
    myGraphUrl += '&hideGrid=true';
  }
  myLocalHideAxes = (graph.hideAxes || myHideAxes);
  if (myLocalHideAxes !== 'false') {
    myGraphUrl += '&hideAxes=true';
  }
  myLocalHideYAxis = (graph.hideYAxis || myHideYAxis);
  if (myLocalHideYAxis !== 'false') {
    myGraphUrl += '&hideYAxis=true';
  }
  myLocalTitleLeft = (graph.titleLeft || myTitleLeft);
  if (myLocalTitleLeft !== '') {
    myGraphUrl += '&vtitle=' + myLocalTitleLeft;
  }
  myLocalTitleRight = (graph.titleRight || myTitleRight);
  if (myLocalTitleRight !== '') {
    myGraphUrl += '&vtitleRight=' + myLocalTitleRight;
  }
  myLocalYMaxLeft = (graph.ymaxLeft || myYMaxLeft);
  if (myLocalYMaxLeft !== '') {
    myGraphUrl += '&yMax=' + myLocalYMaxLeft;
    myGraphUrl += '&yMaxLeft=' + myLocalYMaxLeft;
  }
  myLocalYMaxRight = (graph.ymaxRight || myYMaxRight);
  if (myLocalYMaxRight !== '') {
    myGraphUrl += '&yMaxRight=' + myLocalYMaxRight;
  }
  ...
  reservedParams = [ 'width', 'height', 'targets', 'target',
                     'graphType', 'lineMode', 'areaMode', 'pieMode',
                     'logBase', 'drawNullAsZero',
                     'title', 'titleLeft', 'titleRight',
                     'ymaxLeft', 'ymaxRight',
                     '_salt', 'from', 'until', 'noCache', 'template',
                     'hideLegend', 'hideGrid', 'hideAxes', 'hideYAxis' ];
